...
class Game {
  ...
  ifWin(token) {
    ...
    if (Math.abs(pos[0] - this.flag[0]) <= 24 && Math.abs(pos[1] - this.flag[1]) <= 24) {
      ...
      logger.app.info({ event: 'user.win', token, pos: this.distribution[token] });

      if (Object.keys(this.distribution).length <= 1) {
        logger.app.info({ event: 'user.lose', token: Object.keys(this.distribution)[0] || token });
        this.end();
      } else {
        this.createFlag();
      }
  ...
  start() {
    if (!this.flag && Object.keys(this.distribution).length >= 2) {
      ...
      logger.app.info({ event: 'game.start' });
    }
  }
    ...
    logger.app.info({ event: 'game.end' });
  }
}
